\section{Exploring Structure through Scattering}\label{ch1:sec1:background}
    ...
    \subsection{Scattering at Different Resolution Scales}
        ...
        With this relationship, we can see that by increasing a particle's momentum, its effective wavelength is reduced. This is the fundamental principle which allows electron microscopes to image matter at a resolution of $\sim$ 10--0.1 nm \parencite{Franken2020ADevelopments}, corresponding to electron momenta of $\sim$ 1--100 keV. At this scale, cells \parencite{Williams2009TransmissionMicroscopy}, viruses \parencite{Goldsmith2009ModernViruses}, and even atoms \parencite{Chen2021ElectronVibrations} can be imaged. Other probes could be used to circumvent the diffraction limit, such as high energy (low-wavelength) photons or high momentum (low de Broglie wavelength) protons or neutrons, but electrons are an ideal candidate in this regime as they are easy to produce, steer, interact with the target, and detect. 
    ...
    \subsection{Elastic Scattering and Form Factors}
        ...
        Typical elastic scattering cross sections for transitions metals with 100 keV incident electrons as in electron microscopy are $\sim 10^{-22} \text{m} ^2$ \parencite{Williams2009TransmissionMicroscopy}. In contrast, the cross sections to be discussed in this thesis are on the order of tens of nanobarn \footnote{One barn is defined as $10^{-28} \, \text{m}^2$, approximately the cross-sectional area of a uranium nucleus. One nanobarn is $10^{-9}$ barn or $10^{-37} \, \text{m}^2$.
        } ( $10^{-36} m^2$), or 14 orders of magnitude smaller.         
      ...
      %  In general, as illustrated in Fig. \ref{fig:GeometricScattering} electrons will scatter through an angle $\theta$ into a solid angle $\Omega$.
        ...
        The momentum transfer \textit{q} sets the resolution scale for these processes, but it is convenient to work with the negative square of this value, \eqref{eq:q2}
    ...
    \subsection{Inelastic Scattering and Parton Distribution Functions}
       ...
       \myequations{ $F_2$ structure function in terms of PDFs}
        ...
        The global experimental results can be integrated with theoretical QCD frameworks such as the Dokshitzer–Gribov–Lipatov–Altarelli–Parisi (DGLAP) evolution equations \parencite{Altarelli1977AsymptoticLanguage}, which describe how parton distribution functions change with the scale of the interaction in quantum chromodynamics, to plot PDFs for various constituents of the proton as shown in \figref{fig:PDFPlots}. 
